...
func executeForArgs(args []string) (output string, exit int, err error) {
	...
	caFile := app.Flag("ca-file", "Path to file containing PEM-encoded trusted certificate(s) for the OSN").String()
	...
	configBlockPath := join.Flag("config-block", "Path to the file containing the config block").String()
	...
	if *clientCert != "" {
		if *clientKey == "" {
			return "", 1, errors.New("must specify both --client-cert and --client-key")
		}
