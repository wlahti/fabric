...
var _ = Describe("osnadmin", func() {
	...
	Describe("Flags", func() {
		It("allows short versions of the --orderer-address and --channel-id flags", func() {
			args := []string{
				"channel",
				"remove",
				"-o", ordererURL,
		...
		Context("when the client cert but not the client key is provided", func() {
			BeforeEach(func() {
				clientKey = ""
			})
			...
			It("returns with exit code 1 and prints the error", func() {
				...
				output, exit, err := executeForArgs(args)
				checkFlagError(output, exit, err, "must specify both --client-cert and --client-key")
			})
		})
		...
		Describe("Mutual TLS", func() {
			...
			Context("when the client cert/key are not provided", func() {
				It("returns with exit code 1 and prints the error", func() {
					args := []string{
						"channel",
						"list",
					...
					output, exit, err := executeForArgs(args)
					checkCLIErrorRegExp(output, exit, err, fmt.Sprintf("Get \"%s/participation/v1/channels\": .*: tls: bad certificate", testServer.URL))
				})
			})
		})
