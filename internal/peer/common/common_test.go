...
func TestSetBCCSPConfigOverrides(t *testing.T) {
	...
	t.Run("SW security set to string value", func(t *testing.T) {
		cleanup := setBCCSPEnvVariables(envConfig)
		defer cleanup()
		os.Setenv("CORE_PEER_BCCSP_SW_SECURITY", "WOOPS")

		err := common.SetBCCSPConfigOverrides(bccspConfig)
		require.EqualError(t, err, "CORE_PEER_BCCSP_SW_SECURITY set to non-integer value: WOOPS")
	})
